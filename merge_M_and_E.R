#Author: Alice Carr 
#Function to merge weekly report files, running off Guinea template to be further edited to deal with none templated documents
# define input, output directories and outputname ie. merge_M_and_E("test/","output/","upload")
...
merge_M_and_E <- function(inputdirectory,
  ...
  for (f in 1:base::length(files)) {
    ...
    table_sheet1 <- openxlsx::read.xlsx(wb, sheet = 2, skipEmptyRows = T, colNames = F, rows=1:122, skipEmptyCols=T, detectDates=T)
    if (template==T){ #if template is same, this could go after sheet is loaded in if we dont already know what template countries have used
    ...
    table_sheet1$X2<-ifelse(is.na(table_sheet1$X2),table_sheet1$X6,table_sheet1$X2)
    table_sheet1<-table_sheet1[,-c(3:6)]
    table_sheet1<-table_sheet1[!duplicated(table_sheet1$X1), ]
    
    #Dealing with the observation columns 
    Observations<-na.omit(as.data.frame(table_sheet1$X8))
    #Read in empty template of observations
    #make sure file path correct
    checkobservations<-utils::read.csv("Observations_empty_check.csv")
    
    Observations<-cbind(Observations, checkobservations)
    Observations$trueobservation<-ifelse(Observations$`table_sheet1$X8`==Observations$table_sheet1.X8,NA,
                                         Observations$`table_sheet1$X8`)
    
    list<-data.frame(paste("Observation",seq(1,43), sep="_"))
    Observations<-cbind(Observations,list)
    ...
    upload <- 
      base::cbind("Variable / Field Name" = rownames(upload),upload)
