...
pub_theme4 <- theme(
  ...
  axis.text.x = element_text(size = 28, angle = 45, color = "black", vjust = 0.5, hjust = 0.5),
  ...
  legend.title = element_text(size = 28, face = "bold.italic"),
...
gg1<-df_analyse %>% dplyr::group_by(reporting_date) %>% dplyr::summarise(confirmed=length(grep("Confirmed",finalepiclassification_new,ignore.case = T))) %>%
  ...
  geom_line(aes(x = as.Date(reporting_date), y = round(sevendayavg_overall), colour="#ff3300"), size=3) +
...
topy <- ceiling(
  max(counts$incidence, na.rm = TRUE)
)
...
ggplot(data = counts_plotting, 
  ...
  geom_text_repel(col = "blue", size = 10, 
